...
class AStar():
    def __init__(self, env):
        ...
        self.change_state = env.change_state
    ...
    def reconstruct_path(self, came_from, current):
        ...
        while current in came_from.keys():
            ...
            # 判断转弯
            if (current in came_from.keys()) and (len(total_xy)>2):
                before = came_from[current]
                if self.turn_time(self.location(before), total_xy[-2]):
                    total_xy.append(total_xy[-1])
                    # print(total_xy)
        total_path = self.change_state(total_xy[::-1])
        # print(total_path)
        return total_path
    ...
    def if_turn(self, came_from, current, neighbor):
        ...
        while current in came_from.keys():
            ...
            if abs(neighbor_xy[0]-camefrom_xy[0]) and abs(neighbor_xy[1]-camefrom_xy[1]):
                turn_cost = 1
            break
        return turn_cost

    def turn_time(self, before, current):
        turn = False
        if abs(before[0]-current[0])*abs(before[1]-current[1]):
            turn = True
        return turn
