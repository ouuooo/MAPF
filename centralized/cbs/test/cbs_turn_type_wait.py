...
from a_starturn_fa_turntime import AStar
import time
import random
...
class Environment(object):
    def __init__(self, dimension, agents, obstacles,mainpath):
    ...
    def get_neighbors(self, state):
        ...
        if self.state_valid(n):
            neighbors.append(n)
        if (state.location.x, state.location.y) in self.mainpath:
            # Up action
        ...
        for t in range(max_t):
            ...
            # combinations(iterable, r)-求列表或生成器iterable中指定数目r的元素不重复的所有组合
                ...
                if state_1.is_equal_except_time(state_2):
                    print(agent_1, agent_2, "vetex conflict")
                    result.time = t
            ...
            for agent_1, agent_2 in combinations(solution.keys(), 2):
                ...
                if state_1a.is_equal_except_time(state_2b) and state_1b.is_equal_except_time(state_2a):
                    print(agent_1, agent_2, "edge conflict")
                    result.time = t
    ...
    def compute_solution(self):
        ...
        """
        solution = {}
        for agent in sorted(self.agents, key=lambda item:item['level'], reverse=True):
        # for agent in self.agent_dict.keys():
            # agent = 'agent_name'
            self.constraints = self.constraint_dict.setdefault(agent, Constraints())
            # setdefault(key, value)-设置key的默认值为value。key的值存在时不修改，不存在key时添加，并且其值为value
            # 这里增加判断小车负载类型、货位状态的约束
            # 输入改为单纯的坐标列表，优化计算速度
    ...
    def compute_solution_cost(self, solution):
        ...
        # return max([len(path) for path in solution.values()])

    def change_state(self, solution_xy):
        new_state = []
        for index, p in enumerate(solution_xy):
            new_state.append(State(index, Location(p[0], p[1])))
        return new_state
...
class CBS(object):
    ...
    def search(self):
        ...
        time2 = time.time()
        print('low level search = ', time2-time1)
        ...
        while self.open_set:
            ...
            if not conflict_dict:
                ...
                print('high level search = ', time3-time2)
...
def main():
    ...
    # {'agents': [{'start': [0, 0], 'goal': [2, 0], 'name': 'agent0', 'type': EMPTY}, {'start': [2, 0], 'goal': [0, 0], 'name': 'agent1', 'type': FULL}], 
    # 'map': {'dimensions': [3, 3], 'obstacles': [(0, 1), (2, 1)]}}

    dimension = param["map"]["dimensions"]      # [x_range, y_range]
    obs = find_coordinate(param["map"]["detail"], '.')       # [(obs_x01, obs_y01), (obs_x02, obs_y02)]
    mainpath = find_coordinate(param["map"]["detail"], 'o') 
    agents = param['agents']        # [{'srart': , 'goal': , 'name': }, {'srart': , 'goal': , 'name': }]
    obstacles_full = full_cell(agents, mainpath, obs, dimension)
    print(len(obs), len(obstacles_full))

    env = Environment(dimension, agents, obstacles_full, mainpath)
